...
{
    public class Mintime : BaseModual, ILotteryProduce
    {
        public void GetForecastResult(int periodCount, int count, bool isMulti = false)
        {
            ...
            if (isMulti)
            {
                var isHad = ProduceData.GetForecastByperiodAndType(nextPeriod, "MinTime");
                if (isHad > 0) return;
            }
            ...
            var sql = @"SELECT TOP 6 lottery.redBall ,
                        ...
                        GROUP BY lottery.blueBall1 ORDER BY COUNT";
